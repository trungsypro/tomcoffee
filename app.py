...
def init_db():
    ...
    # Bảng cấu hình tài khoản nhận VietQR (Mới)
...
if choice == "🛒 Gọi Món & Tính Tiền":
    if st.session_state.show_online_invoice:
        ...
        discount_html = ""
        if inv['discount'] > 0:
            discount_html = f"<p style='text-align: right; color: red; margin: 5px 0;'>Giảm gia: -{format_currency(inv['discount'])} đ</p>"

        # Lấy thông tin VietQR từ DB để tạo mã QR động tự động
        ...
        # Tạo URL mã QR chuẩn VietQR (Mã hóa khoảng trắng bằng %20)
        ...
        # CHÚ Ý: Toàn bộ khối mã HTML bên dưới đã được xóa bỏ khoảng cách đầu dòng để tránh lỗi Code Block trên Streamlit
...
# --- CHỨC NĂNG 4: CÀI ĐẶT HỆ THỐNG ---
    ...
    # --- TAB CẤU HÌNH TÀI KHOẢN THANH TOÁN VIETQR ---
    with tab_vietqr:
        ...
        new_account_name = st.text_input("Tên chủ tài khoản (Viết hoa không dấu, ví dụ: NGUYEN TRUNG SY):", value=current_pay[2])

...
# QUẢN LÝ GIỎ HÀNG RIÊNG CHO TỪNG BÀN (Từ Bàn 1 đến Bàn 20)
...
if choice == "🛒 Gọi Món & Tính Tiền":
    ...
    # TRƯỜNG HỢP 1: ĐANG HIỂN THỊ HÓA ĐƠN ONLINE (DO KHÔNG CÓ MÁY IN)
    if st.session_state.show_online_invoice:
        ...
        st.markdown(
            ...
            <div style="background-color: #f8f9fa; padding: 25px; border-radius: 10px; border: 1px dashed #333; max-width: 400px; margin: auto; font-family: 'Courier New', Courier, monospace; color: black;">
                <h2 style="text-align: center; margin-bottom: 5px;">1987 KAFE</h2>
    ...
    # TRƯỜNG HỢP 2: HIỂN THỊ GIAO DIỆN CHÍNH
    else:
        st.header("🗺️ Sơ Đồ Bàn Ghế & Gọi Món")
        
        # HIỂN THỊ TRỰC QUAN 20 BÀN GHẾ (Bố trí dạng lưới 5 cột x 4 hàng)
        ...
        for i in range(1, 21):
            ...
            # Kiểm tra xem bàn hiện tại có đang lưu món nào không
            has_items = len(st.session_state.tables_cart[t_name]) > 0
            
            with table_cols[col_idx]:
                # Thiết kế nhãn hiển thị nút bấm kèm màu sắc phân biệt trạng thái bàn
                btn_label = f"🟢 {t_name} (Trống)" if not has_items else f"🔴 {t_name} (Có khách)"
                
                # Nếu đây là bàn đang được chọn xử lý, đổi kiểu nút để làm nổi bật
        ...
        # KHI KÍCH VÀO MỘT BÀN: HIỂN THỊ MENU GỌI MÓN VÀ GIỎ HÀNG CỦA BÀN ĐÓ
            ...
            if not menu_data:
                    ...
                    for index, cat_name in enumerate(available_categories):
                        with tabs[index]:
                            ...
                            for _, row in df_filtered.iterrows():
                                ...
                                # Lấy số lượng món ăn hiện tại trong giỏ của riêng bàn này
                ...
                with col2:
                    ...
                    if not current_cart:
                        ...
                        if st.button("🔥 Thanh Tốn & In Hóa Đơn", type="primary", key=f"pay_btn_{active_table}"):
                            ...
                            if print_success:
                                ...
                                st.session_state.tables_cart[active_table] = {} # Reset giỏ hàng bàn này về trống
                                st.session_state.selected_table = None
                                st.rerun()
                            else:
                                st.session_state.last_invoice_data = {
                                ...
                                st.session_state.tables_cart[active_table] = {} # Xóa giỏ hàng sau khi tính tiền
                                st.session_state.selected_table = None
...
# --- CHỨC NĂNG 2 VÀ 3 GIỮ NGUYÊN HOÀN TOÀN ---

...
if 'selected_table' not in st.session_state:
    st.session_table = None

def login(username, password):
    # Kiểm tra động trong database (áp dụng cho cả Admin và Nhân viên)
    ...
    if user_data and user_data[0][0] == password:
        ...
        st.session_state.user_role = user_data[0][1] # 'Chủ quán' hoặc 'Nhân viên'
    ...
    if st.session_state.show_online_invoice:
        ...
        for i in range(1, 21):
            ...
            with table_cols[col_idx]:
                ...
                is_selected = (st.session_state.get('selected_table') == t_name)
...
# --- CHỨC NĂNG 4: CÀI ĐẶT TÀI KHOẢN HỆ THỐNG (ĐÃ CẬP NHẬT QUẢN LÝ ADMIN) ---
    ...
    with tab_admin:
        ...
        if st.button("Cập nhật tài khoản Admin", type="primary"):
            if not new_admin_user or not new_admin_pass:
                ...
                conn = sqlite3.connect('cafe_management.db')
                c = conn.cursor()
                # Xóa tài khoản cũ (phòng trường hợp đổi cả username) và chèn tài khoản admin mới
                ...
                try:
                    ...
                    conn.close()
                    # Buộc đăng xuất để đăng nhập bằng thông tin mới
                    logout()

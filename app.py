...
def init_db():
    ...
    # Bảng orders (Doanh thu bán hàng)
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS cash_flow (
                    ...
                    category TEXT NOT NULL,   -- Danh mục thu/chi
                    amount REAL NOT NULL,     -- Số tiền
                    note TEXT,                -- Ghi chú chi tiết
                    created_at TEXT NOT NULL  -- Ngày tháng năm
                )''')
...
# --- CHỨC NĂNG 3: BÁO CÁO DOANH THU ---
    ...
    time_filter = st.radio(
        ...
        horizontal=True, key="report_time_filter"
    )
    ...
    if time_filter == "Hôm nay":
        ...
        with col_d1:
            start_date = st.date_input("Từ ngày:", today - datetime.timedelta(days=7), key="rep_s")
        with col_d2:
            end_date = st.date_input("Đến ngày:", today, key="rep_e")
    ...
    st.markdown("---")
    st.subheader(f"📋 Kết quả báo cáo bán hàng từ ngày {start_date.strftime('%d/%m/%Y')} đến {end_date.strftime('%d/%m/%Y')}:")
...
# --- CHỨC NĂNG VỪA THÊM: QUẢN LÝ SỔ THU CHI LINH HOẠT ---
    ...
    tab_add_flow, tab_report_flow = st.tabs(["📝 Ghi nhận Khoản Thu/Chi mới", "📊 Báo cáo Quản lý Quỹ"])
    
    with tab_add_flow:
        st.subheader("Nhập thông tin Thu / Chi phát sinh")
        
        f_type = st.selectbox("Loại giao dịch:", ["Chi (Chi tiền ra)", "Thu (Thu tiền ngoài vào)"])
        
        if "Chi" in f_type:
            f_cat = st.selectbox("Danh mục Chi:", ["Nhập nguyên vật liệu (Cafe, Đá, Sữa...)", "Chi phí mặt bằng/Điện nước", "Trả lương nhân viên", "Mua sắm trang thiết bị", "Chi khác"])
        else:
            f_cat = st.selectbox("Danh mục Thu ngoài:", ["Thu thanh lý tài sản", "Chủ quán bỏ thêm vốn", "Thu khoản khác"])
            
        f_amount = st.number_input("Số tiền phát sinh (VNĐ):", min_value=0, step=1000)
        f_note = st.text_area("Ghi chú nội dung chi tiết (Ví dụ: Nhập 10kg hạt Robusta, Trả tiền điện tháng 6...):")
        f_date = st.date_input("Ngày phát sinh giao dịch:", datetime.date.today())
        
        if st.button("💾 Lưu vào Sổ thu chi", type="primary"):
            if f_amount <= 0:
                st.error("Vui lòng nhập số tiền lớn hơn 0!")
            else:
                type_db = "Chi" if "Chi" in f_type else "Thu"
    ...
    with tab_report_flow:
        st.subheader("📊 Thống kê tình hình Quỹ Thu/Chi")
        
        flow_filter = st.radio(
            "Xem sổ quỹ theo thời gian:",
            ["Hôm nay", "Hôm qua", "Tuần này", "Tháng này", "Khoảng thời gian tùy chọn"],
            horizontal=True, key="flow_time_filter"
        )
        ...
        if flow_filter == "Hôm nay":
            f_start = t_today
            f_end = t_today
        elif flow_filter == "Hôm qua":
            f_start = t_today - datetime.timedelta(days=1)
            f_end = t_today - datetime.timedelta(days=1)
        elif flow_filter == "Tuần này":
            f_start = t_today - datetime.timedelta(days=t_today.weekday())
            f_end = t_today
        elif flow_filter == "Tháng này":
            f_start = t_today.replace(day=1)
            f_end = t_today
        elif flow_filter == "Khoảng thời gian tùy chọn":
            col_cf1, col_col_cf2 = st.columns(2)
            with col_cf1:
                f_start = st.date_input("Từ ngày:", t_today - datetime.timedelta(days=7), key="cf_s")
            with col_col_cf2:
                f_end = st.date_input("Đến ngày:", t_today, key="cf_e")
        ...
        if loi_nhuan_rong >= 0:
            ...
            c_m3.metric("📉 Thâm Hụt Dòng Tiền", f"{format_currency(loi_nhuan_rong)} đ")
            
        st.markdown("---")
        st.write(f"💵 **Chi tiết Doanh thu bán hàng tự động:** {format_currency(total_sales_amount)} đ")
        
        st.write("📋 **Danh sách các khoản Thu/Chi phát sinh thủ công:**")
        if not flow_data:
            st.info("Không có khoản thu chi ngoài nào trong khoảng thời gian này.")
        else:
            df_flow = pd.DataFrame(flow_data, columns=["Mã phiếu", "Phân loại", "Danh mục", "Số tiền gốc", "Ghi chú nội dung", "Ngày tạo"])
            df_flow_disp = df_flow.copy()
            df_flow_disp["Số tiền (đ)"] = df_flow_disp["Số tiền gốc"].apply(format_currency)

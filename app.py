...
def print_xprinter(table_num, cart_items, total_bill, discount_amount, final_bill):
    try:
        ...
        for name, info in cart_items.items():
            ...
            # Định dạng bill Xprinter giữ nguyên dấu phẩy chuẩn thư viện gốc hoặc tùy biến
...
if choice == "🛒 Gọi Món & Tính Tiền":
    if st.session_state.show_online_invoice:
        ...
        discount_html = ""
        if inv['discount'] > 0:
            discount_html = f"<p style='text-align: right; color: red;'>Giảm giá: -{format_currency(inv['discount'])} đ</p>"
        ...
        st.markdown(
            ...
            <div style="background-color: #f8f9fa; padding: 25px; border-radius: 10px; border: 1px dashed #333; max-width: 400px; margin: auto; font-family: 'Courier New', Courier, monospace; color: black;">
                ...
                <hr style="border-top: 1px dashed #333;">
                <p style="text-align: right;">Tổng tiền gốc: {format_currency(inv['total'])} đ</p>
                {discount_html}
                <h3 style="text-align: right; margin-top: 10px;">THÀNH TIỀN: {format_currency(inv['final'])} đ</h3>
                <hr style="border-top: 1px dashed #333;">
...
# --- CHỨC NĂNG 3: BÁO CÁO DOANH THU ---
    ...
    if not orders_data:
        ...
        # Tạo bản copy định dạng dấu chấm hàng nghìn cho bảng hiển thị đẹp mắt
...
# --- CHỨC NĂNG 4: CÀI ĐẶT HỆ THỐNG ---
    ...
    st.header("⚙️ Cài Đặt Hệ Thống & Cấu Kinh")
